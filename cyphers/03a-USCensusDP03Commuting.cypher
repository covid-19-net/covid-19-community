...
MERGE (c:Commuting{id: 'ACS5-' + row.stateFips + '-' + row.countyFips})
SET c.workers16YearsAndOver = toInteger(row.workers16YearsAndOver),
...
MATCH (c:Commuting{id: 'ACS5-' + row.stateFips + '-' + row.countyFips})
...
MERGE (c:Commuting{id: 'ACS5-' + row.postalCode})
SET c.workers16YearsAndOver = toInteger(row.workers16YearsAndOver),
    c.droveAloneToWorkInCarTruckOrVan = toInteger(row.droveAloneToWorkInCarTruckOrVan),
    c.droveAloneToWorkInCarTruckOrVanPct = toFloat(row.droveAloneToWorkInCarTruckOrVanPct),
    c.carpooledToWorkInCarTruckOrVan = toInteger(row.carpooledToWorkInCarTruckOrVan),
...
MATCH (c:Commuting{id: 'ACS5-' + row.postalCode})
MERGE (e)-[h:HAS_COMMUTING]->(c)
RETURN count(h) as HAS_COMMUTING
;
USING PERIODIC COMMIT
...
MERGE (c:Commuting{id: 'ACS5-' + row.tract})
SET c.workers16YearsAndOver = toInteger(row.workers16YearsAndOver),
    c.droveAloneToWorkInCarTruckOrVan = toInteger(row.droveAloneToWorkInCarTruckOrVan),
    c.droveAloneToWorkInCarTruckOrVanPct = toFloat(row.droveAloneToWorkInCarTruckOrVanPct),
    c.carpooledToWorkInCarTruckOrVan = toInteger(row.carpooledToWorkInCarTruckOrVan),
...
MATCH (c:Commuting{id: 'ACS5-' + row.tract})
MERGE (e)-[h:HAS_COMMUTING]->(c)
RETURN count(h) as HAS_COMMUTING
;