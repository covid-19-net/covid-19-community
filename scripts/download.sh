...
wget --continue --mirror --no-directories ftp://ftp.ncbi.nih.gov/pub/taxonomy/taxdump.tar.gz -P $LDIR  >> "$LOGDIR"/update.log 2>&1
tar -xvzf  "$LDIR"/taxdump.tar.gz -C "$LDIR" >> "$LOGDIR"/update.log 2>&1

#
# CNCB Strain Data
#
echo "*** Downlading CNCB Strain Data ***"  >> "$LOGDIR"/update.log 2>&1

LDIR="$NEO4J_IMPORT"/cache/raw/cncb

for d in a b c d e f g h i j k l m n
do
 echo "Downloading:" ftp://download.big.ac.cn/GVM/Coronavirus/gff3/$d  >> "$LOGDIR"/update.log 2>&1
 wget --continue --mirror --no-directories ftp://download.big.ac.cn/GVM/Coronavirus/gff3/$d -P $LDIR/$d  >> "$LOGDIR"/update.log 2>&1
done
